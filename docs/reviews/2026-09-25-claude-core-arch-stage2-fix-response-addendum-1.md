...
| Research Evidence | owner: `--quick` | PROTO-DEC-0071: researchers, synthesisers and council participants record Evidence with `record --quick`; a full `record` stays mandatory for changes to code, the kernel or protocol tooling. The four researcher and synthesiser prompts now end with `record --quick`; K-launch's operator keeps the full `record`. | `A-research.md`, `B-research.md`, `A-synthesis.md`, `B-synthesis.md`; `.ai/DECISIONS.md` |
| Roles shim | owner-approved, in L's scope | Found when the package was checked against what is in the flow. `protocol-session start` and the SessionStart hook inject the `## Roles` line for the agent name (`protocol-hooks.cjs:306-315`). Six job agents (copilot, grok, kimi, mistral, agy, and the operator kilo) had none, so each would have been told "Ask the owner before starting work". Researchers in `-p` mode cannot ask. codex, gemini, deepseek and claude would have received their CORE-ARCH roles. The owner approved a temporary compatibility shim until frame-aware assignment (SCHEMA-assignment, В-12): six new lines; a research-frame condition on the lines of claude, codex, gemini and deepseek (the two codex lines and the two matching deepseek lines are joined, text unchanged); three closed Open-questions lines moved verbatim to `.ai/ARCHIVE.md`, keeping TASK at 80 lines. Standing roles outside research frames are unchanged. | `.ai/TASK.md` `## Roles`, `.ai/ARCHIVE.md` (append) |
| Role preflight | done | `protocol-session start --agent <name>` for codex, gemini, deepseek, copilot, grok, kimi, mistral, agy, kilo and claude: 10/10 got no "Ask the owner" line, and each role line points to its job or to the council. The 20 files the preflight created (10 journals, 10 runtime files) were removed; the state afterwards equals the state before. | none left |
| `--preflight` | new, read-only | The launcher parses `## Roles` with the hook's own `assignment()` and checks that every job's agent line names its job and that the operator line exists. It writes nothing; on the candidate it gives 9/9. Three pure checks were added. | `launch.cjs` `rolePreflight`, `preflight`; `launch-test.cjs` |
| K-launch gate and canary | owner's gate | Step 0: DeepSeek's non-blocking verdict on package L, named in the owner's launch message. Step 2: `--preflight` after the self-test and `--check`. Step 5: a canary of b-grok (copilot) and b-mistral (vibe), the two clients whose flags PROTO-DEC-0070 changed; the other four start only when both show `WORKING`. No live launch before the non-blocking verdict. | `K-launch.md` |
...
- Candidate, the same machine (Windows 11, node 22.21.0):
  ...
  - After the split, three consecutive full runs gave 70/70, exit 0, about 17 s each. One more fix
    ...
    70/70, and `--pure` 52/52. After `--preflight` was added, three more runs gave 73/73, and
    `--pure` 55/55. `--preflight` itself gives 9/9, and `--dry` accepts the canary's comma lists.
    After every run, no test process, no `colabs-research` worktree and
