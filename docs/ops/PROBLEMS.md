...
- Isolation: nothing else uses `launch.cjs`.
- Correction of 2026-09-25: the council and review runner `run-chain.cjs` is not isolated. Its jobs
  run in the checkout, with the owner's git credentials. Only the prompt rule (no commit, tag or
  push) and copilot's deny rules stop a push; that is Level 0 of the owner's decision, tracked as
  M-8.
- Audit the same day: the remote holds only the known branches, `v2.0.0` equals the coordinator's
  HEAD (da2a738), and every commit of the day is the coordinator's. No job pushed.
- Way out:
  1. the owner decides C-1: done, PROTO-DEC-0077 item 3;
