...
function checkScope(baseline, scopeFile, forbiddenFile, cwd = process.cwd()) {
  ...
  if (!fs.existsSync(resolvedScope)) {
    throw new Error(`Scope file not found: ${resolvedScope}`);
  }
  ...
  const normScopeRel = normalizePath(path.relative(cwd, resolvedScope));
  let normForbiddenRel = null;
  if (forbiddenFile) {
    normForbiddenRel = normalizePath(path.relative(cwd, path.resolve(cwd, forbiddenFile)));
  }

  const touchedPaths = Array.from(new Set([...trackedList, ...untrackedList]))
    .filter(p => p !== normScopeRel && p !== normForbiddenRel);
  ...
  if (forbiddenFile) {
    ...
    if (!fs.existsSync(resolvedForbidden)) {
      throw new Error(`Forbidden paths file not found: ${resolvedForbidden}`);
    }
    forbiddenList = fs
...
function checkIndependence(reviewPath, options = {}) {
  ...
  if (!fs.existsSync(resolvedReview)) {
    throw new Error(`Review file not found: ${resolvedReview}`);
  }
  ...
  if (!reviewerOwner) {
    throw new Error(`Cannot determine Receipt-Owner from review header: ${resolvedReview}`);
  }

  // 2. Read producer's owner name from declared repository inputs only:
  // a) --producer option
  // b) Producer / Candidate field in the review header
  // c) candidate journal's own Evidence owner (--candidate-journal option or review Candidate journal field)
  // NEVER order candidates by filesystem mtime.
  let producerOwner = options.producer || null;

  if (!producerOwner) {
    // Check if review explicitly states candidate / producer owner
    const candidateMatch = reviewContent.match(/(?:^|\n)\s*>?\s*(?:\*\*)?(?:Candidate(?: session)?|Producer|Author session)(?:\*\*)?:\s*`?([a-zA-Z0-9._-]+)`?/i);
    if (candidateMatch) {
      producerOwner = candidateMatch[1].trim();
    }
  }

  let candidateJournalPath = options.candidateJournal || null;
  if (!candidateJournalPath) {
    const journalMatch = reviewContent.match(/(?:^|\n)\s*>?\s*(?:\*\*)?(?:Candidate journal|Producer journal)(?:\*\*)?:\s*`?([^\n`\r]+)`?/i);
    if (journalMatch) {
      candidateJournalPath = journalMatch[1].trim();
    }
  }

  if (!producerOwner && candidateJournalPath) {
    ...
    if (fs.existsSync(jPath)) {
      ...
      if (recMatch) {
        ...
        producerOwner = path.basename(jPath, '.md');
      }
    }
  }

  // Spec Rule 4: Exit 2 if either owner cannot be determined. Unknown is not independent.
  // Never order candidates by mtime; derive producer only from declared repository inputs.
  if (!producerOwner) {
  ...
  // Check TASK.md for implementer role (used to verify reviewer != implementer)
  let implementerRoles = [];
      ...
      for (const line of lines) {
        ...
        if (match) {
          ...
          if (roleDesc.includes('implementer')) {
            implementerRoles.push(match[1].toLowerCase());
          }
        }
      }
    }
  ...
  // Spec Rule 3: Exit 1 if they are equal, or if the review's owner appears as an implementer
  // of the candidate in .ai/TASK.md roles, naming which rule matched.
  ...
  if (implementerRoles.includes(reviewerAgent)) {
    return {
      independent: false,
      exitCode: 1,
      rule: 'REVIEWER_IS_TASK_IMPLEMENTER',
      reviewerOwner,
      producerOwner,
      message: `Independence violation: Reviewer owner '${reviewerOwner}' appears as implementer (${reviewerAgent}) in .ai/TASK.md roles. (PROTO-DEC-0041 item 1)`
    };
  }
